...
import { HttpClient, HttpContext, HttpEvent, HttpHeaders, HttpParams, HttpResponse,HttpParamsOptions } from '@angular/common/http';
import { Injectable, inject } from '@angular/core';
import { Observable, of, delay, finalize, switchMap, tap } from 'rxjs';

import { AppConfigService, AppCommonHttpClientConfig } from '@qide/util/config';
...
export class _HttpClient {
  ...
  private cog: AppCommonHttpClientConfig;
  constructor(cogSrv: AppConfigService) {
    this.cog = cogSrv.merge('commonHttp', {
  ...
  parseParams(params: NonNullable<HttpParamsOptions["fromObject"]>): HttpParams {
    ...
    Object.keys(params).forEach(key => {
      ...
      if (
        ...
        paramValue = dateValueHandling === 'timestamp' ? paramValue.valueOf() : Math.trunc((paramValue as Date).valueOf() / 1000);
      }
  ...
  appliedUrl(url: string, params:  NonNullable<HttpParamsOptions["fromObject"]>): string {
